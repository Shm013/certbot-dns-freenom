...
Build() {
    ...
    pushd "$CONTEXT_PATH"
        DOCKER_TAG="$DOCKER_TAG" CERTBOT_VERSION="$CERTBOT_VERSION" DOCKER_REPO="$DOCKER_REPO" DOCKERFILE_PATH="$DOCKERFILE_PATH" bash hooks/pre_build
        DOCKER_TAG="$DOCKER_TAG" CERTBOT_VERSION="$CERTBOT_VERSION" DOCKER_REPO="$DOCKER_REPO" DOCKERFILE_PATH="$DOCKERFILE_PATH" bash hooks/build
    popd
}
